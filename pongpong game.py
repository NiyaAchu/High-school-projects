...
ball = pygame.Rect(0,0,30,30) #(x,y,width,height)
...
score_font = pygame.font.Font(None,50) #(Font family, font size)

while True:
    for event in pygame.event.get(): #gets all the events in pygame happened since the last time loop ran and puts them in a list
    ...
    #Draw game objects
    screen.fill('black') #so that it does not trace the ball in 60 times

    cpu_score_surface = score_font.render(str(cpu_points), True, 'white') #score points in str, anti allias, colour
